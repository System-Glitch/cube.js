...
const generateQueryTypes = async (apiUrl: string, currentToken: string) => {
  ...
  const pathToCubeRuntime = require.resolve('@cubejs-client/runtime');
  const pathToDefinitionFile = pathToCubeRuntime.replace('/src/index.js', '/generated.d.ts');
...
export function configureTypegenCommand(program: CommanderStatic): void {
  program
    .command('typegen <apiUrl> <token>')
    .description('Generate types from your Cube API for your frontend project')
    .action(
      (apiUrl, token) => generateQueryTypes(apiUrl, token)
